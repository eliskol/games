...
class HeuristicMinimaxStrategy:
    ...
    def generate_tree(self, board_state, n):
        ...
        for state in self.node_dict:
            if state not in self.terminal_nodes and hasattr(
                self.node_dict[state], "minimax_value"
            ):
    ...
    def propagate_minimax_values(self):
        ...
        for node in self.terminal_nodes:
            node.minimax_value = (
                {1: 9999, 2: -9999, "Tie": 0}[node.winner]
                if node.winner is not None
                else self.assign_heuristic_value(node.state)
            )
            if (
                self.tree.deeptuple(node.state) not in self.calculated_heuristic_values
                and self.save_heuristic_values
            ):
                print("updating cache dict")
                self.need_to_update_pickle = True
                self.calculated_heuristic_values[
                    self.tree.deeptuple(node.state)
                ] = node.minimax_value
    ...
    def choose_move(self, board):
        ...
        print(
            "after propagation: cache dict length is",
            len(self.calculated_heuristic_values),
        )
        ...
        if board == [[0 for _ in range(7)] for _ in range(6)]:
            ...
            == 1
            != sum([row.count(2) for row in board])
        ):
            self.player = 2
            return 2
        ...
        if goal_node.winner is not None and self.save_heuristic_values:
            with open("heuristic_values.pickle", "wb") as f:
                ...
                print(len(self.calculated_heuristic_values))
                pickle.dump(
                    self.calculated_heuristic_values, f, pickle.HIGHEST_PROTOCOL
                )
    ...
    def calculate_heuristic_value(self, board: list[list[int]]):
        ...
        for i in range(6):
            for j in range(4):
                ...
                if (
                    horizontal.count(1) == 3 or horizontal.count(2) == 3
                ) and horizontal.count(0) == 1:
                    heuristic_value += {3: 0.9, 0: -0.9}[horizontal.count(1)]
                elif (
                    horizontal.count(1) == 2 or horizontal.count(2) == 2
                ) and horizontal.count(0) == 2:
                    heuristic_value += {2: 0.3, 0: -0.3}[horizontal.count(1)]
                elif (
                    horizontal.count(1) == 1 or horizontal.count(2) == 1
                ) and horizontal.count(0) == 3:
                    heuristic_value += {1: 0.1, 0: -0.1}[horizontal.count(1)]
        for j in range(7):
            for i in range(3):
                vertical = [board[i + k][j] for k in range(4)]
                if (vertical.count(1) == 3 or vertical.count(2) == 3) and vertical[
                    -1
                ] == 0:
                    heuristic_value += {3: 0.9, 0: -0.9}[vertical.count(1)]
                elif (vertical.count(1) == 2 or vertical.count(2) == 2) and vertical[
                    -2
                ] == 0:
                    heuristic_value += {2: 0.3, 0: -0.3}[vertical.count(1)]
                elif (vertical.count(1) == 1 or vertical.count(2) == 1) and vertical[
                    -3
                ] == 0:
        ...
        for i in range(0, 3):
            for j in range(0, 4):
                ...
                if positive_diagonal.count(0) == 0 and (
                    positive_diagonal.count(1) == 0 or positive_diagonal.count(2) == 0
                ):
                    heuristic_value += {
                        True: [0, 0.1, 0.3, 0.9][positive_diagonal.count(1)],
                ...
                if negative_diagonal.count(0) == 0 and (
                    ...
                    heuristic_value += {
                        True: [0, 0.1, 0.3, 0.9][negative_diagonal.count(1)],
