...
for _ in range(10000):
    ...
    start = time.time()
    filled_in_spaces2 = list(
        filter(lambda index: example_game_state[index[0]][index[1]] != 0, indexes)
    )
    end = time.time()
...
print(
    "filter"
    if sum(comprehension_times) / len(comprehension_times)
    > sum(filter_times) / len(filter_times)
    else "comp"
)
